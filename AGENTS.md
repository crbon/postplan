...
The CLI API shapes in `convex/http.ts` must remain compatible with `bin/postplan.js`. The CLI state lives in `~/.postplan`. Draft IDs and the `<prefix>/<draftId>/<sha256>.html` key format must remain stable so re-uploads preserve document links.
